...
// React app entry generated by the Vite frontend build. React updates metadata
// after it loads, but crawlers and "View Source" need the canonical in the
// initial server response too.
$serveReactApp = function () {
    ...
    $html = preg_replace(
        '~(<meta\\s+[^>]*\\bproperty=["\\\']og:url["\\\'][^>]*\\bcontent=["\\\'])[^"\\\']*(["\\\'][^>]*>)~i',
        '$1'.$canonical.'$2',
        $html
    );
...
// The React app uses browser-history routes. These URLs must return the app
// entry point on a direct request or browser refresh; client-side navigation
// alone never reaches Laravel, which is why the issue only showed on refresh.
...
foreach ([
    ...
    '/blogs',
    '/hsn-finder',
    '/clusters/herbal-ingredients',
    '/clusters/jaggery-honey',
    '/clusters/marble-inlay',
    '/clusters/glass-textile',
] as $path) {
    Route::get($path, $serveReactApp);
}

// Blog articles are dynamic client-side routes.
Route::get('/blog/{slug}', $serveReactApp)->where('slug', '[A-Za-z0-9-]+');

// Retain real 404s for invalid paths. This prevents search engines from
// receiving a 200 response for URLs that do not exist in the React app.
