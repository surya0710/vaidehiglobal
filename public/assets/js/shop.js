...
//             skipValues[0].innerText = values[0];
//             skipValues[1].innerText = values[1];
...
// };
  ...
  /* Range Two Price
  -------------------------------------------------------------------------------------*/
  function rangeTwoPrice() {
    var skipSlider = document.getElementById("price-value-range");
    if (skipSlider && !skipSlider.noUiSlider) {
        var min = parseInt(skipSlider.dataset.min);
        var max = parseInt(skipSlider.dataset.max);

        noUiSlider.create(skipSlider, {
            start: [
                parseInt(document.getElementById("min_price_input").value) || min,
                parseInt(document.getElementById("max_price_input").value) || max
            ],
            connect: true,
            step: 1,
            range: {
        ...
        skipSlider.noUiSlider.on("update", function (values) {
            document.getElementById("price-min-value").textContent = values[0];
            document.getElementById("price-max-value").textContent = values[1];
